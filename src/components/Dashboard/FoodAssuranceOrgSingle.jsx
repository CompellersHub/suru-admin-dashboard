...
const FoodAssuranceOrgSingle = () => {
  ...
  const [membersOrderData, setMembersOrderData] = useState([]);
  ...
  useEffect(() => {
    ...
    setMembersOrderData(singleOrgData?.membersOrder || []);
  }, [singleOrgData]);
  ...
  // Helper function to get order count for a member
  const getOrdersForMember = (memberId) => {
    const memberData = membersOrderData.find(
      (item) => item.userId === memberId
    );
    return memberData?.orders || [];
  };

  // Helper function to calculate total order amount for a member
  const getTotalOrderAmount = (memberId) => {
    const orders = getOrdersForMember(memberId);
    return orders.reduce((total, order) => total + (order.bill || 0), 0);
  };
          ...
          <table className="rounded-md overflow-hidden w-full">
            ...
            <thead className="bg-green-100">
              <tr className="text-navbar-color py-2 h-14">
                ...
                <th className="p-2 text-center">Order Placed</th>
              </tr>
            </thead>
            ...
            <tbody className="mt-5 bg-white text-[#3A3A3A]">
              {filteredOrg &&
                filteredOrg?.slice(page, page + 10).map((item) => {
                  const orderCount = getOrdersForMember(item?._id).length;
                  const totalOrderAmount = getTotalOrderAmount(item?._id);
