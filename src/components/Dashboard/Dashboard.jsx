...
const Dashboard = () => {
  ...
  const { mutateAsync: fetchVendors, isPending } = useFetchVendors()
  const [page, setPage] = useState(0)
  const {
    data: singleOrderData,
    isFetching: singleLoading,
    refetch,
  } = useFetchSingleVendor(selectVendorId, { enabled: false })
          ...
          <tbody className='mt-5 bg-white text-[#3A3A3A]'>
            {filteredVendors?.slice(page, page + 10).map((item) => (
              <tr
                ...
                className='text-center cursor-pointer mt-5 py-2 h-12 border-b-[1px] border-green-200 hover:bg-slate-100 hover:cursor-pointer'
              >
